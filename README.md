...
**阶段：M0 技术验证、M1 项目登记与只读浏览、M2 变化感知与终端、M3 文件操作与登记生命周期均已交付，10 项验收场景全部闭环；剩余 Windows 分发实测、真实桌面人工确认，以及 8 项「设计建议」层的未落地项（不影响已确认需求 C01–C10）。**
...
| 已完成 | 推进计划 v1（决策门与决策记录、里程碑任务清单、风险登记 R1–R10、**§ 12 完成度审查**）、技术边界研究快照 |
...
**验证现状：18 个套件、605 项断言；各套件断言均通过**（2026-09-21 逐条复跑复现，含静态检查、
类型检查与构建；明细见下文「验证」）。
...
| `npm run verify:m1` | 登记、去重、视图状态、文件浏览与预览、网络图片授权的登记侧 | 61 |
...
| `npm run smoke:m1` | 端到端（真实 Electron + 构建产物 + 界面交互，含文件操作、重新定位、退出确认、三阶段启动位置、网络图片开关、文件树键盘导航、提示条键盘关闭、模态聚焦、tabpanel 关联） | 128 |
...
**合计 605 项断言。** 端到端套件在真实 Electron 中加载 `out/` 构建产物，通过渲染进程
