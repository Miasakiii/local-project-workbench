...
**验证现状：17 个套件、596 项断言；各套件断言均通过**（2026-09-21 逐条复跑复现，含静态检查、
类型检查与构建；明细见下文「验证」）。
...
| `npm run verify:remote-image` | 网络图片授权与主进程代理：授权门、地址形态、本机／内网、连接前域名解析复核、响应与重定向复核、体积与超时 | 50 |
...
**合计 596 项断言。** 端到端套件在真实 Electron 中加载 `out/` 构建产物，通过渲染进程
