...
**阶段：M0 技术验证、M1 项目登记与只读浏览、M2 变化感知与终端、M3 文件操作与登记生命周期均已交付；剩余 Windows 分发实测与完整回归。**
...
**验证现状：12 个套件、436 项断言；各套件断言均通过**（明细见下文「验证」）。
...
| [M3 验证记录](docs/plan/M3-验证记录.md) | 文件操作、批量失败报告、重新定位与信任重确认、退出前会话提示的验证结果与已知限制 |
...
| `npm run verify:m3-1` | 同一父目录内单点重命名（文件／文件夹、冲突、信任、重解析点） | 21 |
| `npm run verify:m3-file-ops` | 新建、复制／剪切粘贴、删除、回收站降级与批量失败报告 | 31 |
| `npm run verify:m3-lifecycle` | 重新定位与信任重确认、退出前活动会话提示 | 36 |
| `npm run smoke:m1` | 端到端（真实 Electron + 构建产物 + 界面交互，含文件操作、重新定位与退出确认） | 94 |
| `npm run measure:m0-7` | 冷启动、内存与体积实测 | — |

**合计 436 项断言。** 端到端套件在真实 Electron 中加载 `out/` 构建产物，通过渲染进程
实际调用 IPC 并驱动界面交互（展开目录、拖拽尺寸、切换项目、查看差异、触发文件监听、
执行文件操作、重新定位、退出确认），并断言计算后的样式以防「样式表未生效」类回归。
