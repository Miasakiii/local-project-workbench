...
**验证现状：14 个套件、564 项断言；各套件断言均通过**（2026-09-21 逐条复跑复现，含静态检查、
类型检查与构建；明细见下文「验证」）。
...
- **对外请求只有一个出口**：远程图片由主进程代取，先复核项目授权（读登记记录，不接受界面声明）、
  协议、是否指向本机或内网、响应类型与体积，**重定向后的地址再复核一遍**，之后才转 data URL；
...
- 网络图片默认不加载，按项目授权；已授权者由主进程代取，CSP 的 `img-src 'self' data:` 不放宽
- 终端输入与文档预览分离权限与脚本上下文
...
| `npm run verify:remote-image` | 网络图片授权与主进程代理：授权门、地址形态、本机／内网、响应与重定向复核、体积与超时 | 43 |
| `npm run smoke:m1` | 端到端（真实 Electron + 构建产物 + 界面交互，含文件操作、重新定位、退出确认、三阶段启动位置与网络图片开关） | 124 |
| `npm run measure:m0-7` | 冷启动、内存与体积实测 | — |

**合计 564 项断言。** 端到端套件在真实 Electron 中加载 `out/` 构建产物，通过渲染进程
