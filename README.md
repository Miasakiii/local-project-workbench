...
**阶段：M0 技术验证、M1 项目登记与只读浏览、M2 变化感知与终端、M3 文件操作与登记生命周期均已交付，10 项验收场景全部闭环；界面重构三项（文件栏右键化 / 侧边栏设置入口 / 终端主角化）已按设计分四阶段落地，并在真实桌面完成人工验收。剩余 Windows 分发实测，以及 8 项「设计建议」层的未落地项（不影响已确认需求 C01–C10）。**
...
同日补齐其中的 **G1：网络图片按项目授权**（设计稿 4.3）。默认行为不变——仍然不加载；
补上的是「用户可按项目允许」这一半。远程图片**由主进程代理抓取**并复核协议、类型、体积、
超时与重定向后的地址，转成 data URL 才交给界面，因此渲染层 CSP 未放宽、界面从不直连远程地址。
新增 43 项纯 Node 断言、10 项登记侧断言与 12 项端到端断言，详见
[推进计划 § 11.7](docs/plan/推进计划-v1.md) 与[M3 验证记录 § 九](docs/plan/M3-验证记录.md)。

同日完成一轮**界面重构三项**（[设计](docs/plan/界面重构三项-设计.md)已批准，按四阶段落地）：
文件栏约 15 个按钮下沉为**渲染层自绘右键菜单**（回到设计稿 4.2/6.1 的右键本意，并提供
Shift+F10／Menu 键盘等价键）；侧边栏左下角新增**设置**入口与设置页，收拢 编辑器路径 /
默认终端 Shell（新增白名单契约）/ 恢复上次项目 / 关于；**终端主角化**——头部主按钮、
Ctrl+\`、会话存在即自动展开、可全屏，**只揭示既有会话、绝不自动创建**。
写守卫、回收站、信任门与会话归属一律未动。详见[推进计划 § 13](docs/plan/推进计划-v1.md)。

| 状态 | 事项 |
|---|---|
| 已完成 | 设计讨论稿 **v0.4**（实施基线；M0–M2 的实施偏差已回写） |
| 已完成 | 推进计划 v1（决策门与决策记录、里程碑任务清单、风险登记 R1–R10、**§ 12 完成度审查**、**§ 13 界面重构三项**）、技术边界研究快照 |
...
**验证现状：19 个套件、667 项断言；各套件断言均通过**（2026-09-21 逐条复跑复现，含静态检查、
类型检查与构建；明细见下文「验证」）。
...
- **项目库首页**：卡片、搜索、置顶、最近打开、简介（README 首段 > 路径回退；卡片「编辑简介」
  可填写自定义简介，清空保存即恢复自动）。
- **文件树**：点击文件夹整行即展开／收起，子目录按需加载；只读预览 Markdown、纯文本、
  代码（含行号与轻量语法高亮）、常见栅格图片；二进制、编码不支持、无权限分别说明。
  **针对某一条目的操作一律在右键菜单里**（预览、用默认程序打开、用指定编辑器打开、
  在所在目录新建终端、复制路径、在资源管理器中定位、复制、剪切、重命名、删除），
  工具栏只留视图级操作与「新建」；焦点行按 Shift+F10／Menu 键可开出同一套菜单。
- **README**：按固定顺序识别（兼容大小写，含 `docs`／`.github`），支持多语言变体切换与
  用户指定介绍文件；没有 README 时显示空状态与选择入口，不擅自创建文件。
  网络图片默认不加载；需要时**按项目**授权，开关就在 README 的「被阻止的内容」旁边，
  授权后由主进程代取并复核，界面从不直连远程地址。
- **启动位置**：默认停留在项目库首页（C09）。「恢复上次项目」开关开启后，重启直接进入
  上次活跃的项目；目录已不可用或登记已移除时回到项目库并说明原因，不静默换页。
  恢复的是**浏览位置**，不恢复进程、也不附带写权限（未信任项目仍以只读浏览打开）。
  开关与上次活跃项目都记在应用数据目录的 `settings.json`（应用属性，不写进用户项目）。
- **设置页**（侧边栏左下角入口）：收拢应用级偏好——编辑器路径（用于「用指定编辑器打开」）、
  终端默认 Shell（白名单 pwsh／PowerShell／cmd，缺省自动探测）、启动时恢复上次项目、
  关于（Electron／Chromium／Node 版本）。**不放** per-project 设置（信任、允许网络图片、
  README、简介）——那些属于某个项目，放在项目页里才不会在切换项目时误导用户。
...
- **终端主角化**（界面重构三项·阶段 4）：头部主按钮「终端」+ Ctrl+\` 一键呼出，
  有会话的项目切回来时面板自动展开（**只揭示既有会话，绝不自动创建**），
  可全屏把终端当主视图用；可见性进视图状态，重启后恢复。
...
- **写操作的安全边界**：未信任项目一律拒绝**写操作与终端创建**（终端按登记表 `trusted` 判定，渲染层声明的字段不参与判定）；项目根与 `.git` 元数据受保护，
...
- **Markdown 隔离由输出形态保证**：净化层永不写出可加载的 URL 属性
  （图片用 `data-asset`、已授权远程图片用 `data-remote`、外链用 `data-external-url`、
  项目内链接用 `data-project-path`），因此「输出中不存在 `src`」是可断言的不变量。
- **对外请求只有一个出口**：远程图片由主进程代取，先复核项目授权（读登记记录，不接受界面声明）、
  协议、是否指向本机或内网（字面量与连接前解析出的每个 IP 各复核一次）、响应类型与体积（含魔数嗅探）；
  随后**固定用已校验的公网 IP 建立连接**、并**手动逐跳重定向**（每跳重走全部复核），闭合「校验解析 vs 连接解析」的
  DNS 重绑定与 302 跳转型，之后才转 data URL；渲染层 CSP 维持 `img-src 'self' data:`，未因该功能放宽。默认关闭，按项目授权。
...
| [推进计划 v1](docs/plan/推进计划-v1.md) | 决策门与决策记录、技术选型、目标架构与模块拆分、里程碑任务清单、风险登记 R1–R10、**§ 11.7 网络图片授权边界**、**§ 12 完成度审查与设计稿未落地项清单（G1–G9）** |
...
| [M3 验证记录](docs/plan/M3-验证记录.md) | 文件操作、批量失败报告、重新定位与信任重确认、退出前会话提示、`.git` 归一化绕过修正、「恢复上次项目」开关、**§ 九 网络图片按项目授权与主进程代理**的验证结果与已知限制 |
| [界面重构三项-设计](docs/plan/界面重构三项-设计.md) | 文件栏右键化 / 侧边栏设置入口 / 终端主角化的设计与分阶段实现记录（已落地，见推进计划 § 13） |
...
- 网络图片默认不加载，按项目授权；已授权者由主进程代取、**连接前复核域名解析**，CSP 的 `img-src 'self' data:` 不放宽
- 终端输入与文档预览分离权限与脚本上下文
...
```bash
npm run verify:all          # 静态检查 + 类型检查 + 构建 + M0 + M1 + M2 + M3 + 网络图片 + 端到端（推荐）
```
...
**合计 667 项断言。** 端到端套件在真实 Electron 中加载 `out/` 构建产物，通过渲染进程
实际调用 IPC 并驱动界面交互（展开目录、拖拽尺寸、切换项目、查看差异、触发文件监听、
执行文件操作、重新定位、退出确认、开启「恢复上次项目」、勾选「允许本项目加载网络图片」），
...
- 已知问题 I-2：Windows 上 node-pty 会话被 kill 之后，ConPTY 可能仍短暂持有启动目录，
  `verify:terminal-trust` 清理样例目录时因此报 EPERM。该套件把清理做成「尽力而为」：
  断言跑完才清理，删不掉就只删文件、留存目录并在下一轮复用，同时如实打印残留路径——
  **清理失败不冒充验证失败**（与 I-1 同源的平台行为，非被测逻辑缺陷）。
