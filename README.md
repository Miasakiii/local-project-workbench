...
**阶段：M0 技术验证、M1 项目登记与只读浏览、M2 变化感知与终端、M3 文件操作与登记生命周期均已交付，10 项验收场景全部闭环；界面重构三项（文件栏右键化 / 侧边栏设置入口 / 终端主角化）已按设计分四阶段落地，并在真实桌面完成人工验收。Windows 分发实测已完成：NSIS 安装包 99.9 MB，无人值守安装、静默卸载、用户数据保留与安装目录产物验证（9/9）全部通过。设计稿「设计建议」层 8 项（G2–G9）已全部定夺（补实现或定性关闭），不影响已确认需求 C01–C10。仅剩一项环境受限项：真实 ACL 拒绝删除的场景 7 分支补测（需可配置 ACL 的桌面环境，本开发环境无法构造）。**

M1／M2 的全部实现已于 2026-09-19 纳入版本历史，并完成一次实施审计与修正
（含引入静态检查工具链、主进程 IPC 按域拆分）。

2026-09-21 修正一处安全边界缺陷：受保护项判定此前只看未归一化的原始输入，`sub/../.git`
可绕过并导致 `.git` 被送入回收站。四段写操作守卫已收敛为单一入口 `resolveForWrite()`，
归一化后再查一次受保护项，并补充了对应的断言覆盖。
...
早期验证记录中把它们当作已完成来表述的地方。
已确认需求 C01–C10 与 10 项验收场景的口径经核对**属实**。
其中 8 项（G2–G9）已于同日逐条定夺：G2–G6 补实现，G7／G8 定性关闭，G9 实测达标。
...
对应的断言覆盖（纯 Node、登记侧与端到端）已随该功能一并补齐。

同日完成一轮**界面重构三项**（设计已批准，按四阶段落地）：
...
写守卫、回收站、信任门与会话归属一律未动。

| 状态 | 事项 |
|---|---|
| 已完成 | 设计讨论稿 **v0.4**（实施基线；M0–M2 的实施偏差已回写） |
| 已完成 | 工程基线：Biome 静态检查与格式化、主进程／渲染进程分离类型检查、主进程 IPC 按域拆分（来源校验在 guard） |
...
| 已完成 | **G1（设计稿 4.3）**：网络图片按项目授权，默认不加载；已授权者由主进程代取并复核后转 data URL |
| 已完成 | **界面重构三项**：文件栏右键化 / 侧边栏设置入口+设置页 / 终端主角化（分四阶段落地并各自回归） |
| 已完成 | M3-5 分发实测（2026-09-23）：`pack:win` 生成 NSIS 安装包 **99.9 MB**，无人值守安装／静默卸载／用户数据保留／安装目录上 9/9 项产物验证全部通过 |
| 进行中 | M3-5 余项：真实 ACL 拒绝删除未补测（2026-09-23 本环境尝试未成——本环境不执行 ACL deny，无法构造真实拒绝场景；仍待可配置 ACL 的桌面环境） |
| 已完成 | 设计稿「设计建议」层 8 项（G2–G9）全部定夺：G2–G6 补实现、G7／G8 定性关闭、G9 性能目标实测达标 |
...
**验证现状：19 个套件、667 项断言；各套件断言均通过**（2026-09-23 逐条复跑复现，含静态检查、
类型检查与构建；明细见下文「验证」）。
